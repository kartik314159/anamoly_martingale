...
//				+ " M(n): " + martingaleValue);
	...
	public static void main(String[] args) {
		double index = 0.92;
		List<Double> martingaleValues = new ArrayList<Double>();
		List<Double> pValues = new ArrayList<Double>();
		martingaleValues.add(new Double(1.0));
		for (int i = 0; i < 100; ++i) {
			double pVal = (i < 95) ? 0.05 : 0.85; 
			double martingaleValue = index * Math.pow(pVal, index - 1) * martingaleValues.get(martingaleValues.size() - 1);
			martingaleValues.add(new Double(martingaleValue));
			pValues.add(new Double(pVal));
		}
		
		for (Double m : martingaleValues)
